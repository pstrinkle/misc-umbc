...
#define NUM_FOLDERS 100 //number of folders
...
{
	FILE *stream;
	int  done, n = NUM_FOLDERS, m= STOPWORDS_CNT,i, j, k, k2;
	float **data, **matrix(), **symmat, **symmat2, *vector(), *evals, *interm;
	void free_matrix(), free_vector(), corcol(), covcol(), scpcol();
	void tred2(), tqli();
	float in_value;
	char option='V', names[n+1][MAX_PATH_LENGTH];

        //Added the following to recieve the data from the workers
	int numprocs,myid;
	MPI_Status status;
...
#ifdef PRINT_STATUS
	printf("I am the print server %d.\n", myid);
#endif 
	bzero(&(user_terms), sizeof(user_terms));

	// wait for a username of done
	// the distro server will send this
	// just get the data and print it out to a file
	i = 1;
	// handler the results from each of the clients
	while (!done) {
		MPI_Recv(&(user_terms), 1, userCounts, MPI_ANY_SOURCE, PRINTTAG, world, &status);

#ifdef PRINT_STATUS
		printf("PrintServer Received User: %s\n", user_terms.user);
#endif 
		if (DISTROSERVER == status.MPI_SOURCE) {
			done = 1;
		} else { // was this legit data
			strncpy(names[i],user_terms.user,1024);    //  Analysis option 
			for (j = 0; j < STOPWORDS_CNT; j++) {
				//				printf("%d ", user_terms.counts[i]);
