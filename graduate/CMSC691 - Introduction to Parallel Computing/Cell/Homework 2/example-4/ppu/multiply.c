...
int main(int argc, char *argv[])
{
   ...
   // perform multiply
   printf(
      "SPE: Multiply   elements=%d  bytes=%d\n",
      tsize*tsize,
      tsize*tsize*sizeof(Amatrix[0][0]));
   
   gettimeofday(&time0, &tzone);
   
   // start jobs
   rows = (((tsize/32)+NUM_THREADS-1)/NUM_THREADS)*32 ;
   ...
   // Check for correctness of final matrix
   {
	 ...
	 for(j = 0; j < tsize; j++)
	 {
      ...
      {
	 printf(
	    "Error in Multiply [%d,%d]. -- %f %f\n",
	    fsterror1,
	    fsterror0,
	    Cmatrix[fsterror1][fsterror0],
	    Dmatrix[fsterror1][fsterror0]);
      }
      else
      {
