...
struct timeval time0,time1,time2,time2b,time3;
int sec, usec ;
...
//}
	...
	gettimeofday( &time3, &tzone );

	// print time to complete
	sec  = time1.tv_sec  - time0.tv_sec ;
	usec = time1.tv_usec - time0.tv_usec ;
	if ( usec < 0 ) { sec--; usec+=1000000 ; }
	printf("PPE: Phase 1 -- matrix[%d]: time=%d.%06d  tasks=%d \n"
	,tsize,sec,usec,NUM_THREADS);

	sec  = time2.tv_sec  - time1.tv_sec ;
	usec = time2.tv_usec - time1.tv_usec ;
	if ( usec < 0 ) { sec--; usec+=1000000 ; }
	printf("PPE: Phase 2 -- matrix[%d]: time=%d.%06d  tasks=%d \n"
	,tsize,sec,usec,NUM_THREADS);

	sec  = time2b.tv_sec  - time2.tv_sec ;
	usec = time2b.tv_usec - time2.tv_usec ;
	if ( usec < 0 ) { sec--; usec+=1000000 ; }
	printf("PPE: Phase 3a -- matrix[%d]: time=%d.%06d  tasks=2 \n"
	,tsize,sec,usec);

	sec  = time3.tv_sec  - time2b.tv_sec ;
	usec = time3.tv_usec - time2b.tv_usec ;
	if ( usec < 0 ) { sec--; usec+=1000000 ; }
	printf("PPE: Phase 3b -- matrix[%d]: time=%d.%06d  tasks=1 \n"
	,tsize,sec,usec);

	sec  = time3.tv_sec  - time2.tv_sec ;
	usec = time3.tv_usec - time2.tv_usec ;
	if ( usec < 0 ) { sec--; usec+=1000000 ; }
	printf("PPE: Phase 3 -- matrix[%d]: time=%d.%06d  tasks=%d \n"
	,tsize,sec,usec,NUM_THREADS);

	sec  = time3.tv_sec  - time0.tv_sec ;
	usec = time3.tv_usec - time0.tv_usec ;
	if ( usec < 0 ) { sec--; usec+=1000000 ; }
