...
//printf("Rows:  %d %d %d \n",rows,tsize,NUM_THREADS);
	for ( i=0; i < NUM_THREADS; i++ ) {
		...
		// set arguments
		args[i].initial   = (float (*)[tsize][tsize])initial_matrix ;
		args[i].final     = (float (*)[tsize][tsize])final_matrix ;
		args[i].i_initial = i*rows ;
		if ( (i+1)*rows > tsize ) args[i].i_final = tsize ;
		else                      args[i].i_final = (i+1)*rows ;
