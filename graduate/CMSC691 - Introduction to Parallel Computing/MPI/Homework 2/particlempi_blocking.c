...
int main( int argc, char *argv[] )
{
   ...
   else if (myid == serverA)
   {
	 ...
	 /* copy the data off the temporary buffer */
	 memcpy(
	    &(pos[status.MPI_SOURCE * block]),
	    &(buf[0]),
	    4 * block * sizeof(float));

	 i++;
      }

      MPI_Send(&(pos[0]), 4 * block * 2, MPI_FLOAT, serverB, TOB, world);
   }
   else
   {
      unsigned int i, j, x;
	 ...
	 {
	    memcpy(&(pos[0]), &(buf[0]), 4 * block * 2 * sizeof(float));
	 }
	 else
	 {
	    memcpy(
